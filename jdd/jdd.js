...
var jdd = {
  ...
  validateInput: function (json, side) {
    // First check if it's a URL - if so, don't validate as JSON
    try {
      const url = new URL(json.trim());
      return true;
    } catch (e) {
      // Not a URL, continue with JSON validation
    }
  ...
  compare: function () {
     ...
     */
    var leftValid = jdd.validateInput(
      document.getElementById("textarealeft").value,
      jdd.LEFT
    );
    var rightValid = jdd.validateInput(
    ...
    // Try to parse as URLs first
    var left = null;
    var right = null;

    try {
      const leftUrl = jdd.urlToJson(
        document.getElementById("textarealeft").value.trim()
      );
      if (leftUrl) {
        left = leftUrl;
      }
    } catch (e) {
      // If URL parsing fails, we'll try JSON parse below
    }

    try {
      const rightUrl = jdd.urlToJson(
        document.getElementById("textarearight").value.trim()
      );
      if (rightUrl) {
        right = rightUrl;
      }
    ...
    // If not URLs, try parsing as JSON
    if (!left) {
      try {
        left = JSON.parse(document.getElementById("textarealeft").value);
      } catch (e) {
        document.getElementById("errorLeft").textContent =
    ...
    if (!right) {
      try {
        right = JSON.parse(document.getElementById("textarearight").value);
      } catch (e) {
        document.getElementById("errorRight").textContent =
          "Invalid JSON: " + e.message;
        document.getElementById("errorRight").style.display = "block";
  ...
  /**
   * Convert a URL string into a JSON object with _host, _path and GET parameters
   */
  urlToJson: function (urlStr) {
    try {
      const url = new URL(urlStr);
      const result = {
        _protocol: url.protocol.replace(":", ""), // Remove the trailing colon
        _host: url.host,
        _path: url.pathname,
      };

      // Handle GET parameters, converting duplicates into arrays
      const params = {};
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (jdd.getParameterByName("left")) {
    document.getElementById("textarealeft").value =
      jdd.getParameterByName("left");
  }

  if (jdd.getParameterByName("right")) {
    document.getElementById("textarearight").value =
      jdd.getParameterByName("right");
  }
